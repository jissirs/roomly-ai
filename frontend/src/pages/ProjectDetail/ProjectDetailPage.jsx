...
function ProjectDetailPage() {
  ...
  useEffect(() => {
    ...
    getProject(id).then((loaded) => {
      if (cancelled) return
      setProject(loaded)
      setAiInstructions(loaded?.aiInstructions ?? loaded?.notes ?? '')
    }).finally(() => !cancelled && setIsLoading(false))

    getProjectImages(id).then((images) => {
      if (cancelled) return
      setImageCount(images.length)
      if (images.length) setHeroImage(images[0].url)
    }).catch(() => {})
  ...
  if (isLoading) {
    return <div className="detail-shell"><main className="detail-content"><p>กำลังโหลดโปรเจกต์...</p></main></div>
  }
  ...
  async function handleDelete() {
    await deleteProjectImages(project.id).catch(() => {})
    await removeProject(project.id)
    navigate('/home')
  }

  async function saveAiInstructions(continueAfterSave = false) {
    await updateProject(project.id, { aiInstructions: aiInstructions.trim() })
    ...
    <div className="detail-shell">
      ...
      <main className="detail-content">
        <div className="detail-hero">
          {heroImage ? (
            <img src={heroImage} alt={`ภาพห้องของโปรเจกต์ ${project.name}`} />
          ) : (
